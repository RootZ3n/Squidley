...
async function readReceipts(receiptsDir: string, limit: number): Promise<any[]> {
  ...
  parsed.sort((a, b) => (String(a?.created_at ?? "") < String(b?.created_at ?? "") ? 1 : -1));
...
export async function registerTokenMonitorRoutes(
  ...
  // Summary
  app.get("/skills/token-monitor/summary", async (req, reply) => {
    ...
    for (const r of receipts) {
      ...
      by_provider[provider] ??= { tokens_in: 0, tokens_out: 0, tokens_total: 0, cost: 0 };
      addToAgg(by_provider[provider], ts);

      const modelKey = `${provider}::${model}`;
      by_model[modelKey] ??= { tokens_in: 0, tokens_out: 0, tokens_total: 0, cost: 0 };
      addToAgg(by_model[modelKey], ts);

      by_day[day] ??= { tokens_in: 0, tokens_out: 0, tokens_total: 0, cost: 0 };
  ...
  // Top receipts
  app.get("/skills/token-monitor/top", async (req, reply) => {
    const url = new URL(req.url, "http://127.0.0.1");
    const limitRaw = url.searchParams.get("limit");
