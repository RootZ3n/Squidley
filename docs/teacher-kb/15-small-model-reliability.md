...
are useful, but they need guardrails. Peh's reliability layer adds
...
That is the nature of small models. Peh does not pretend the small
model is bigger than it is — instead, she works *with* it.

## What Peh does to stay reliable
...
2. **Token budgeting.** Peh never dumps a whole large file into
...
3. **Bounded retries.** If a step fails, Peh tries again — once.
   Not forever. The retry limit is fixed in policy.

4. **Decompose on failure.** If the same error keeps happening, or if
   the retry budget runs out, Peh stops looping and suggests
   smaller next steps instead.

5. **Optional cloud escalation.** If local keeps failing, Peh can
