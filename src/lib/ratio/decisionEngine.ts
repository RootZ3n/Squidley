...
import { getRatioModulePolicy } from "./modulePolicies";
import { resolveRatioModelCapability } from "./modelCapabilities";
import { unlockMeets } from "./unlockLevels";
import { beginnerMessageForStatus } from "./explanations";
import type {
  RatioCapabilityLevel,
  RatioDecision,
  RatioDecisionInput,
  RatioDecisionStatus,
  RatioEffectiveMode,
...
export function decideRatioAction(input: RatioDecisionInput): RatioDecision {
  ...
  const policy = getRatioModulePolicy(input.moduleId, input.actionId);
  if (!policy) {
    return decision(input, profile, "future", false, "No Ratio policy is wired for this action.", ["Action policy is not defined yet."]);
  }
  ...
  if (!isDeterministicAction(input.actionId) && !input.modelId?.trim()) {
    return decision(input, profile, "needs-stronger-model", false, "This action needs a selected local model before Ratio can use it.", ["Requires selected model."]);
  }
  ...
  const status: RatioDecisionStatus = input.promptGatewayRisk === "medium" || input.promptGatewayRisk === "high"
    ? "limited"
    : "available";
...
function decision(
  input: RatioDecisionInput,
  profile: RatioModelCapabilityProfile,
  status: RatioDecisionStatus,
  allowed: boolean,
  reason: string,
  safetyNotes: string[],
  requiredUnlockLevel?: RatioDecision["requiredUnlockLevel"],
): RatioDecision {
  ...
  return {
    ...
    modelSummary: deterministic
      ? "This module does not need a model. Ratio treats it as deterministic local behavior."
...
function capabilityLevel(profile: RatioModelCapabilityProfile): RatioCapabilityLevel {
...
function missingCapabilities(
  policy: NonNullable<ReturnType<typeof getRatioModulePolicy>>,
  profile: RatioModelCapabilityProfile,
): string[] {
