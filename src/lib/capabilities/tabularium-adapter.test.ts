...
import {
  ...
  createCapabilityDecisionTabulariumReceipt,
  moduleIdToTabulariumModule,
} from "./tabularium-adapter";
import {
...
describe("capability tabularium adapter — module mapping", () => {
  it("maps known canonical Tabularium modules straight through", () => {
    for (const id of [
      "colloquium",
      "velum",
      "archivum",
      ...
      expect(moduleIdToTabulariumModule(id)).toBe(id);
    }
  });

  it("maps more-input to archivum (its canonical owner)", () => {
    expect(moduleIdToTabulariumModule("more-input")).toBe("archivum");
  });

  it("maps locked / future modules without a Tabularium slot to system", () => {
    for (const id of [
      "archelon",
      "tabularium",
      "legatus",
...
describe("capability tabularium adapter — preserves required fields", () => {
  it("preserves capabilityId, moduleId, tier, state, providerMode, providerId, modelId, flags, escalationReason, createdAt", () => {
    ...
    const input = capabilityDecisionToTabulariumReceiptInput(event);
  ...
  it("uses summary prefix matching the runtime state", () => {
    expect(
      capabilityDecisionToTabulariumReceiptInput(localReadyEvent()).summary,
    ).toMatch(/^Local ready/);
    expect(
      capabilityDecisionToTabulariumReceiptInput(cloudRequiredPendingEvent()).summary,
    ).toMatch(/^Cloud required/);
    expect(
      capabilityDecisionToTabulariumReceiptInput(blockedEvent()).summary,
    ).toMatch(/^Blocked/);
  });
});

describe("capability tabularium adapter — passes through createTabulariumReceipt", () => {
  it("produces a TabulariumReceipt with localOnly=true, cloudUsed=false, modelUsed=false", () => {
    const event = localReadyEvent();
    const receipt = createCapabilityDecisionTabulariumReceipt(event, {
  ...
  it("the adapter input is accepted by createTabulariumReceipt directly", () => {
    const input = capabilityDecisionToTabulariumReceiptInput(cloudRequiredPendingEvent(), {
      receiptId: "fixed-id-2",
    });
    const direct = createTabulariumReceipt(input);
    const viaHelper = createCapabilityDecisionTabulariumReceipt(
  ...
  it("maps more-input events to module=archivum on the resulting receipt", () => {
    const receipt = createCapabilityDecisionTabulariumReceipt(moreInputEvent(), {
  ...
  it("maps locked/future capability events to module=system but keeps original moduleId in metadata", () => {
    const receipt = createCapabilityDecisionTabulariumReceipt(blockedEvent(), {
...
describe("capability tabularium adapter — purity", () => {
  ...
  it("does not call fetch and does not write to localStorage", () => {
    for (const event of [
      ...
      capabilityDecisionToTabulariumReceiptInput(event);
      createCapabilityDecisionTabulariumReceipt(event, { receiptId: "purity-x" });
    }
...
describe("capability tabularium adapter — worst-case metadata", () => {
  it("preserves all 13 capability decision metadata fields, including providerId and modelId", () => {
    ...
    // gets dropped now that the Tabularium metadata cap is widened.
...
describe("capability tabularium adapter — does not mutate the source event", () => {
  it("snapshots the event before adapter and verifies equality after", () => {
    ...
    capabilityDecisionToTabulariumReceiptInput(event);
    createCapabilityDecisionTabulariumReceipt(event, { receiptId: "no-mutate" });
