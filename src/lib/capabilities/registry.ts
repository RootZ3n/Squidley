...
export const CAPABILITIES: readonly Capability[] = [
  ...
  },
  {
    ...
    receiptActions: ["notebook.local-storage"],
    velumGated: false,
  },
  {
  ...
  },
  {
    ...
    beginnerDescription:
      "Paste outside text into Notebook's local storage. Stays in this browser only.",
    tier: "local-core",
    localRequirements: [],
    cloudRequirements: [],
    honestMessages: {
      localReady:
        "More Input writes to Notebook's local storage in this browser. No model or upload is involved.",
    },
    receiptActions: ["notebook.local-storage"],
    velumGated: false,
  },
  ...
  },
  {
    ...
    receiptActions: ["vision.local-image-analysis"],
    velumGated: true,
  },
  {
  ...
  },
  {
    ...
    honestMessages: {
      cloudRequired:
        "ControlPanel controls require future cloud unlock plus explicit high-trust permissions. Locked in public mode.",
    },
    receiptActions: "none",
    velumGated: true,
  },
 ...
 */
export function findUnregisteredAssessmentActions(): { moduleId: string; actionId: string }[] {
  const missing: { moduleId: string; actionId: string }[] = [];
  for (const m of PUBLIC_MODULES) {
    if (m.assessmentActions === "none") continue;
    for (const action of m.assessmentActions) {
