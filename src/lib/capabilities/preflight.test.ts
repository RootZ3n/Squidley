...
} from "@/lib/colloquium/capabilityReceipts";
...
describe("buildCapabilityPreflightReceiptInput", () => {
  it("creates a Tabularium-compatible capability.decision receipt input", () => {
...
describe("recordCapabilityPreflightReceipt", () => {
  ...
  it("handles storage failure gracefully without throwing", () => {
    ...
    // logTabulariumReceipt swallows storage errors internally and still
...
describe("Tabularium badge detection works for generic preflight receipts", () => {
...
describe("Colloquium helper produces identical metadata after delegating", () => {
  it("Colloquium build helper output matches generic preflight for the same inputs", () => {
    const colloquiumInput = buildColloquiumCapabilityDecisionReceiptInput({
      createdAt: 500,
      localChatReady: true,
      providerId: "ollama",
      modelId: "llama3.1:8b",
