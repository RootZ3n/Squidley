...
import {
  ...
  cloudEscalationOfferToTabulariumReceiptInput,
  createCloudEscalationOfferReceiptEvent,
  createCloudEscalationOfferTabulariumReceipt,
...
import { TABULARIUM_MAX_METADATA_ENTRIES } from "@/lib/tabularium/receipts";
...
describe("cloudEscalationOfferToTabulariumReceiptInput", () => {
  it("uses action cloud-escalation.offer", () => {
    const event = createCloudEscalationOfferReceiptEvent(cloudRequiredPacket());
    const input = cloudEscalationOfferToTabulariumReceiptInput(event);
  ...
  it("metadata stays within Tabularium cap", () => {
    const event = createCloudEscalationOfferReceiptEvent(cloudRequiredPacket());
    const input = cloudEscalationOfferToTabulariumReceiptInput(event);
    const keys = Object.keys(input.metadata!);
    expect(keys.length).toBeLessThanOrEqual(TABULARIUM_MAX_METADATA_ENTRIES);
  });
...
describe("createCloudEscalationOfferTabulariumReceipt", () => {
  it("creates a valid TabulariumReceipt", () => {
    const event = createCloudEscalationOfferReceiptEvent(cloudRequiredPacket());
    const receipt = createCloudEscalationOfferTabulariumReceipt(event);
...
describe("forbidden content keys are absent", () => {
  ...
  it("Tabularium input has no forbidden keys", () => {
    const event = createCloudEscalationOfferReceiptEvent(cloudRequiredPacket());
    const input = cloudEscalationOfferToTabulariumReceiptInput(event);
    assertNoForbiddenKeysDeep(input.metadata as unknown as Record<string, unknown>);
  });

  it("Tabularium receipt has no forbidden keys in metadata", () => {
    const event = createCloudEscalationOfferReceiptEvent(cloudRequiredPacket());
    const receipt = createCloudEscalationOfferTabulariumReceipt(event);
    if (receipt.metadata) {
...
describe("cloud escalation receipt helpers — purity", () => {
  ...
  it("no fetch/provider/cloud calls occur", () => {
    ...
    const event = createCloudEscalationOfferReceiptEvent(packet);
    cloudEscalationOfferToTabulariumReceiptInput(event);
    createCloudEscalationOfferTabulariumReceipt(event);
