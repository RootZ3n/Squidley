...
import {
  ...
  findUnregisteredRatioActions,
  getCapabilitiesForModule,
} from "./registry";
import { CAPABILITY_TIERS } from "./contracts";

describe("public capability registry", () => {
  it("registers exactly one capability per ratioActions entry across PUBLIC_MODULES", () => {
    const missing = findUnregisteredRatioActions();
    ...
    for (const m of PUBLIC_MODULES) {
      ...
      totalActions += m.ratioActions.length;
    }
