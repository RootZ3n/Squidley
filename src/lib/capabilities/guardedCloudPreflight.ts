...
import { createCapabilityDecisionTabulariumReceipt } from "./tabularium-adapter";
...
import { capabilityDecisionToTabulariumReceiptInput } from "./tabularium-adapter";
...
export interface GuardedCloudPreflightReceipts {
  capabilityReceipt: TabulariumReceipt | null;
  assessmentReceipt: TabulariumReceipt | null;
  policyReceipt: TabulariumReceipt | null;
  offerReceipt: TabulariumReceipt | null;
}
...
function recordPreflightReceipts(
  ...
  try {
    ...
    const receiptInput = capabilityDecisionToTabulariumReceiptInput(event);
    result.capabilityReceipt = logTabulariumReceipt(storage, receiptInput);
  } catch {
