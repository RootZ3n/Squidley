...
describe("recordCloudEscalationOfferAndDecision", () => {
  ...
  it("if storage is broken, both writes are still attempted by default", () => {
    // logTabulariumReceipt swallows storage errors internally and returns
    ...
    // Both are "recorded" from logTabulariumReceipt's perspective (it
  ...
  it("attemptDecisionOnOfferFailure=false skips decision when offer throws", () => {
    ...
    // does (logTabulariumReceipt catches), so this path only activates
    // if offer returns null. Since logTabulariumReceipt swallows errors
