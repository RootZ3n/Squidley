...
import {
  ...
  cloudConsentToTabulariumReceiptInput,
  createCloudConsentReceiptEvent,
  createCloudConsentTabulariumReceipt,
...
describe("cloudConsentToTabulariumReceiptInput", () => {
  it("uses the correct action per decision", () => {
    for (const decision of ["granted", "denied", "cancelled"] as const) {
      const event = createCloudConsentReceiptEvent(actionablePacket(), decision, { createdAt: 1 });
      const input = cloudConsentToTabulariumReceiptInput(event);
...
describe("createCloudConsentTabulariumReceipt", () => {
  it("creates a valid TabulariumReceipt", () => {
    const event = createCloudConsentReceiptEvent(actionablePacket(), "granted", { createdAt: 1 });
    const receipt = createCloudConsentTabulariumReceipt(event);
...
describe("cloud consent receipt helpers — purity", () => {
  ...
  it("no fetch/provider/cloud calls occur", () => {
    ...
    cloudConsentToTabulariumReceiptInput(event);
    createCloudConsentTabulariumReceipt(event);
