...
describe("browser storage wrappers", () => {
  ...
  it("saves and clears the expected key", () => {
    ...
    expect(storage.setItem.mock.calls[0][0]).toBe(COLLOQUIUM_STORAGE_KEY);
    expect(storage.setItem.mock.calls[0][1]).toContain('"localOnly":true');

    clearStoredConversation(storage);
    expect(storage.removeItem).toHaveBeenCalledWith(COLLOQUIUM_STORAGE_KEY);
  });
});
...
describe("local chat sessions", () => {
  ...
  it("adapts old v1 conversation storage into the first session", () => {
    ...
    const storage = {
      getItem: (key: string) => (key === COLLOQUIUM_STORAGE_KEY ? v1 : null),
    };
  ...
  it("clears all v1 and v2 local chat keys", () => {
    ...
    expect(storage.removeItem).toHaveBeenCalledWith(COLLOQUIUM_SESSIONS_STORAGE_KEY);
    expect(storage.removeItem).toHaveBeenCalledWith(COLLOQUIUM_STORAGE_KEY);
  });
  ...
  it("saves v2 sessions to the v2 key", () => {
    ...
    expect(storage.setItem.mock.calls[0][0]).toBe(COLLOQUIUM_SESSIONS_STORAGE_KEY);
    expect(storage.setItem.mock.calls[0][1]).toContain('"localOnly":true');
  });
