...
export function loadStoredSessions(
  ...
  try {
    const v2 = deserializeSessionsDocument(storage.getItem(CHAT_SESSIONS_STORAGE_KEY), now);
    if (v2) return v2;

    const v1 = deserializeConversation(storage.getItem(CHAT_STORAGE_KEY), now);
...
export function saveStoredSessions(
  ...
  try {
    storage.setItem(CHAT_SESSIONS_STORAGE_KEY, serializeSessionsDocument(doc, now));
  } catch {
...
export function loadStoredConversation(
  ...
  try {
    return deserializeConversation(storage.getItem(CHAT_STORAGE_KEY), now);
  } catch {
    return emptyConversation();
  }
}
