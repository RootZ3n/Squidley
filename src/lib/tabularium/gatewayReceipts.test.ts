...
import {
  ...
  tabulariumReceiptUrl,
} from "./gatewayReceipts";
import { loadTabularium } from "./receipts";
...
describe("gateway receipts", () => {
  ...
  it("prevents duplicate receipts with the same dedupe key", () => {
    ...
    expect(loadTabularium(s).receipts).toHaveLength(1);
  });

  it("builds safe Tabularium receipt URLs without raw prompt text", () => {
    const url = tabulariumReceiptUrl("gateway-colloquium-r:1-prompt-gateway-blocked");

    expect(url).toContain("/tabularium?receipt=");
    expect(url).not.toContain(" ");
    expect(url).toContain("r%3A1");
    expect(tabulariumReceiptUrl("ignore previous instructions token=sk-123")).toBe("/tabularium");
