...
export interface GatewayReceiptArgs {
  module: Extract<TabulariumModule, "colloquium" | "fabrica" | "oculus" | "system">;
  route: string;
  metadata: unknown;
  modelUsed: boolean;
  dedupeKey: string;
...
export function createPromptGatewayReceipt(args: GatewayReceiptArgs): TabulariumReceipt | null {
...
export function tabulariumReceiptUrl(receiptId: string): string {
  const safeId = sanitizeReceiptId(receiptId);
  return safeId ? `/tabularium?receipt=${encodeURIComponent(safeId)}` : "/tabularium";
}
