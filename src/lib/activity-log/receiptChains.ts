/**
 * ActivityLog receipt chain / grouping helpers.
 *
 * Pure helpers that group related ActivityLog receipts into readable trust
...
import type { ActivityReceipt } from "./receipts";
import {
...
export type ActivityReceiptChainKind =
  | "cloud-consent-flow"
...
export type ActivityReceiptStepKind =
  | "assessment"
  | "policy"
  | "offer"
  | "consent"
...
export interface ActivityReceiptChainStep {
  receiptId: string;
  action: string;
  module: string;
  title: string;
  ...
  stepKind: ActivityReceiptStepKind;
}

export interface ActivityReceiptChain {
  id: string;
  kind: ActivityReceiptChainKind;
  title: string;
  summary: string;
  createdAt: number;
  updatedAt: number;
  steps: readonly ActivityReceiptChainStep[];
  riskLevel?: string;
...
export function classifyReceiptChainStep(
  receipt: Readonly<ActivityReceipt>,
): ActivityReceiptStepKind {
...
function receiptToStep(receipt: Readonly<ActivityReceipt>): ActivityReceiptChainStep {
...
function classifyChainKind(stepKinds: ReadonlySet<ActivityReceiptStepKind>): ActivityReceiptChainKind {
...
const CHAIN_TITLES: Record<ActivityReceiptChainKind, string> = {
...
function extractRiskLevel(steps: readonly ActivityReceiptChainStep[], receipts: readonly ActivityReceipt[]): string | undefined {
...
function extractFinalDecision(steps: readonly ActivityReceiptChainStep[], receipts: readonly ActivityReceipt[]): string | undefined {
...
function computeNothingSentYet(steps: readonly ActivityReceiptChainStep[], receipts: readonly ActivityReceipt[]): boolean {
  for (const step of steps) {
...
export function summarizeReceiptChain(chain: Readonly<ActivityReceiptChain>): string {
...
export function buildReceiptChains(
...
): ActivityReceiptChain[] {
  const windowMs = options.timestampWindowMs ?? 5000;

  // Phase 1: group receipts by shared key
  const groups = new Map<string, ActivityReceipt[]>();
  const ungrouped: ActivityReceipt[] = [];
  ...
  const chains: ActivityReceiptChain[] = [];
  ...
  for (const [groupKey, group] of groups) {
    ...
    const chain: ActivityReceiptChain = {
      id: `chain-${groupKey}`,
      kind,
  ...
  for (const receipt of ungrouped) {
    ...
    const stepKinds = new Set<ActivityReceiptStepKind>([step.stepKind]);
    const kind = classifyChainKind(stepKinds);
...
export function chainContainsForbiddenContentKeys(
  chain: Readonly<ActivityReceiptChain>,
): boolean {
