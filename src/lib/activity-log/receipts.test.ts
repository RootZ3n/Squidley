...
import {
  ACTIVITY_LOG_MAX_METADATA_ENTRIES,
  ...
  deserializeActivity,
  filterActivityReceipts,
  formatActivityExport,
  loadActivity,
  logActivityReceipt,
  sanitizeReceiptText,
  saveActivity,
  updateActivityReceiptStatus,
} from "./receipts";

describe("ActivityLog receipts", () => {
  ...
  it("serializes and deserializes safely", () => {
    ...
    const restored = deserializeActivity(JSON.stringify(createActivityDocument([receipt], 200)));
  ...
  it("falls back for invalid or corrupt storage", () => {
    expect(deserializeActivity("{bad")).toMatchObject({ receipts: [] });
    expect(deserializeActivity(JSON.stringify({ version: 99 }))).toMatchObject({ receipts: [] });
  });
  ...
  it("appends and updates receipt status", () => {
    ...
    const doc = appendActivityReceipt(createActivityDocument([], 1), receipt, 2);
    const updated = updateActivityReceiptStatus(doc, "chat-1", "succeeded", {
      completedAt: 10,
  ...
  it("filters by search, module, status, and model use", () => {
    const chat = createActivityReceipt({
      id: "chat",
      module: "chat",
    ...
    const velum = createActivityReceipt({
      id: "velum",
      module: "velum",
      action: "review",
      status: "info",
    ...
    expect(filterActivityReceipts({ receipts, query: "streamed" }).map((r) => r.id)).toEqual(["chat"]);
    expect(filterActivityReceipts({ receipts, module: "velum" }).map((r) => r.id)).toEqual(["velum"]);
    expect(filterActivityReceipts({ receipts, status: "succeeded" }).map((r) => r.id)).toEqual(["chat"]);
    expect(filterActivityReceipts({ receipts, model: "no-model" }).map((r) => r.id)).toEqual(["velum"]);
  });

  it("formats export with local-only header", () => {
    const exported = formatActivityExport(createActivityDocument([], 1), "2026-04-25T00:00:00.000Z");
    expect(exported).toContain("Peh Public Activity Log Export");
  ...
  it("loads, saves, logs, and clears with the expected storage key", () => {
    ...
    expect(loadActivity(storage).receipts).toEqual([]);
    saveActivity(storage, createActivityDocument([], 1));
    expect(storage.setItem.mock.calls[0][0]).toBe(ACTIVITY_LOG_STORAGE_KEY);
    const logged = logActivityReceipt(storage, {
      module: "settings",
      action: "tour.restart",
    ...
    expect(loadActivity(storage).receipts).toHaveLength(1);
    expect(clearActivityReceipts().receipts).toEqual([]);
  });
  ...
  it("supports Workshop receipts without storing full source or output", () => {
    const receipt = createActivityReceipt({
      module: "workshop",
      action: "single-file-suggestion",
      status: "succeeded",
      title: "Workshop suggestion completed",
      summary: "Workshop created a local single-file suggestion. No files were written and no commands were run.",
      provider: "local",
      model: "llama3.2",
      modelUsed: true,
  ...
  it("exposes a metadata cap as a named constant set to 16", () => {
    expect(ACTIVITY_LOG_MAX_METADATA_ENTRIES).toBe(16);
  });

  it("caps metadata entries at ACTIVITY_LOG_MAX_METADATA_ENTRIES and keeps the earliest entries", () => {
    const oversized: Record<string, string | number | boolean> = {};
    for (let i = 0; i < ACTIVITY_LOG_MAX_METADATA_ENTRIES + 4; i++) {
    ...
    expect(keys).toHaveLength(ACTIVITY_LOG_MAX_METADATA_ENTRIES);
    expect(keys[0]).toBe("field00");
    expect(keys[keys.length - 1]).toBe(`field${String(ACTIVITY_LOG_MAX_METADATA_ENTRIES - 1).padStart(2, "0")}`);
    expect(keys).not.toContain(`field${String(ACTIVITY_LOG_MAX_METADATA_ENTRIES).padStart(2, "0")}`);
  });
