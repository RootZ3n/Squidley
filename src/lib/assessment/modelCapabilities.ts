...
type PartialProfile = Omit<AssessmentModelCapabilityProfile, "providerId" | "providerType" | "modelIdPattern"> & {
  pattern: RegExp;
...
const CLOUD_PREPARED: Record<Exclude<ProviderId, "ollama" | "llama-cpp">, AssessmentModelCapabilityProfile> = {
...
export function resolveAssessmentModelCapability(args: {
  ...
  return {
    ...
    confidence: "low",
    modelSummary:
      "This local model is unknown to Assessment, so Peh treats it conservatively. It may be useful for simple chat, but not for agent work.",
