...
describe("local model task suitability", () => {
  ...
  it("never permits local multi-file builds or tool agents", () => {
    expect(evaluateLocalModelForTask({ taskId: "fabrica.multi-file-build", model: { name: "qwen3-coder:30b" } }).status).toBe("needs-cloud-unlock");
  ...
  it("summarizes all task rows for a model", () => {
    ...
    expect(rows.map((row) => row.taskId)).toContain("fabrica.multi-file-build");
