...
import { createNotebookDocument, createNotebookEntry, formatNotebookBundle, importNotebookBundle, parseNotebookBundle } from "@/lib/notebook/storage";
...
describe("public release safety contract", () => {
  ...
  it("keeps malformed Notebook imports from replacing local entries", () => {
    const existing = createNotebookEntry({ id: "existing", type: "note", text: "Keep me", now: 1 });
    const doc = createNotebookDocument([existing], 2);

    expect(parseNotebookBundle("{not json")).toBeNull();
    expect(parseNotebookBundle(JSON.stringify({ entries: [] }))).toBeNull();
    expect(doc.entries.map((entry) => entry.id)).toEqual(["existing"]);
  });

  it("imports Notebook bundles without overwriting same-id local entries", () => {
    const local = createNotebookEntry({ id: "same", type: "note", text: "local", now: 1 });
    const incoming = createNotebookEntry({ id: "same", type: "note", text: "incoming", now: 2 });
    const preview = parseNotebookBundle(formatNotebookBundle(createNotebookDocument([incoming], 3)));
    expect(preview).not.toBeNull();

    const result = importNotebookBundle(createNotebookDocument([local], 4), preview!.bundle, 100);
  ...
  it("keeps Vision local, refuses non-vision models clearly, and saves only analysis text", async () => {
  ...
  it("creates searchable receipts without leaking likely secrets", () => {
    const receipt = createActivityReceipt({
      ...
      metadata: { rawToken: "ghp_abcdefghijklmnopqrstuvwxyz", route: "/notebook" },
    });
    ...
    expect(filterActivityReceipts({ receipts: [receipt], module: "notebook", query: "entry" })).toEqual([receipt]);
  });
