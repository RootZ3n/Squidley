...
describe("colloquium tour", () => {
  it("is registered under the moduleId", () => {
    expect(getTour("colloquium")).toBe(colloquiumTour);
  });

  it("has unique step ids", () => {
    const ids = colloquiumTour.steps.map((s) => s.id);
    expect(new Set(ids).size).toBe(ids.length);
  });

  it("first step explains the Latin meaning of Colloquium", () => {
    const first = colloquiumTour.steps[0];
  ...
  it("covers every required UI region", () => {
    const targets = colloquiumTour.steps.map((s) => s.target);
  ...
  it("every step has a non-empty title and body", () => {
    for (const s of colloquiumTour.steps) {
  ...
  it("has at least 6 steps (intro + 6 UI regions)", () => {
    expect(colloquiumTour.steps.length).toBeGreaterThanOrEqual(6);
  });
  ...
  it("first step body explicitly says Latin for conversation or discussion", () => {
    const body = colloquiumTour.steps[0].body.toLowerCase();
