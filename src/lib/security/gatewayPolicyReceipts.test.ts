...
import {
  ...
  createGatewayPolicyDecisionTabulariumReceipt,
  gatewayPolicyDecisionToTabulariumReceiptInput,
...
// ---------------------------------------------------------------------------
// Tabularium adapter — per boundary
// ---------------------------------------------------------------------------

describe("gatewayPolicyDecisionToTabulariumReceiptInput", () => {
  it("uses correct action", () => {
    ...
    const input = gatewayPolicyDecisionToTabulariumReceiptInput(event);
  ...
  it("metadata stays within Tabularium cap", () => {
    ...
    const event = createGatewayPolicyDecisionReceiptEvent(decision);
    const input = gatewayPolicyDecisionToTabulariumReceiptInput(event);
    expect(Object.keys(input.metadata!).length).toBeLessThanOrEqual(TABULARIUM_MAX_METADATA_ENTRIES);
  });
...
describe("createGatewayPolicyDecisionTabulariumReceipt", () => {
  it("creates a valid TabulariumReceipt", () => {
    const decision = evaluateGatewayPolicyForChat(assess("Hi"));
    const event = createGatewayPolicyDecisionReceiptEvent(decision);
    const receipt = createGatewayPolicyDecisionTabulariumReceipt(event);
...
describe("gateway policy receipt helpers — purity", () => {
  ...
  it("no fetch/provider/model/cloud calls occur", () => {
    ...
    gatewayPolicyDecisionToTabulariumReceiptInput(event);
    createGatewayPolicyDecisionTabulariumReceipt(event);
