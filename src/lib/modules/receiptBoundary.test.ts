...
const INLINE_RECEIPT_PATTERNS = [
  /\blogActivityReceipt\s*\([^)]*,\s*\{/s,
  /\baddActivityReceipt\s*\([^)]*,\s*\{/s,
  /\bsaveActivityReceipt\s*\([^)]*,\s*\{/s,
];

describe("receipt ownership boundary", () => {
  it("keeps ActivityLog receipt payload construction out of app page files", () => {
    ...
    expect(
      offenders,
      [
        "Use a module-owned receipt builder instead of constructing ActivityLog payloads inline in page files.",
        "Allowed flow: module receipt builder -> ActivityLog log/store helper -> UI display.",
