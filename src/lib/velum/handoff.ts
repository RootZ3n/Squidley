...
export interface ColloquiumToVelumHandoffPayload {
  version: typeof VELUM_HANDOFF_VERSION;
  createdAt: number;
  source: "colloquium";
  localOnly: true;
...
export function consumeVelumHandoff(
  ...
  try {
    ...
    try {
      ...
      // Ignore storage failures; handoff should never crash Colloquium.
    }
  }
...
export function createColloquiumToVelumPayload(
  draftText: string,
  now = Date.now(),
): ColloquiumToVelumHandoffPayload | null {
...
export function parseColloquiumToVelumPayload(
  raw: string | null,
  now = Date.now(),
): ColloquiumToVelumHandoffPayload | null {
  if (typeof raw !== "string" || raw.trim().length === 0) return null;
  ...
  const payload = parsed as Partial<ColloquiumToVelumHandoffPayload> | null;
  if (
    !payload ||
    payload.version !== VELUM_HANDOFF_VERSION ||
...
export function saveColloquiumToVelumHandoff(
  storage: Pick<Storage, "setItem">,
  draftText: string,
  now = Date.now(),
): boolean {
  const payload = createColloquiumToVelumPayload(draftText, now);
  if (!payload) return false;
  try {
    storage.setItem(COLLOQUIUM_TO_VELUM_HANDOFF_KEY, JSON.stringify(payload));
    return true;
  } catch {
    return false;
  }
...
export function consumeColloquiumToVelumHandoff(
  storage: Pick<Storage, "getItem" | "removeItem">,
  now = Date.now(),
): ColloquiumToVelumHandoffPayload | null {
  let raw: string | null = null;
  try {
    raw = storage.getItem(COLLOQUIUM_TO_VELUM_HANDOFF_KEY);
  } catch {
    return null;
  } finally {
    try {
      storage.removeItem(COLLOQUIUM_TO_VELUM_HANDOFF_KEY);
    } catch {
      // Ignore storage failures; handoff should never crash Velum.
    }
  }
  return parseColloquiumToVelumPayload(raw, now);
}
