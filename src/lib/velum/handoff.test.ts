...
import {
  ...
  createChatToVelumPayload,
  createVelumHandoffPayload,
  mergeVelumDraft,
  parseChatToVelumPayload,
...
describe("Chat to Velum handoff payloads", () => {
  it("creates a browser-local draft review payload without URL involvement", () => {
    const payload = createChatToVelumPayload("draft with possible secret", 100);
  ...
  it("validates version, expiry, and malformed data", () => {
    expect(parseChatToVelumPayload("{bad", 100)).toBeNull();
    expect(
      parseChatToVelumPayload(
        JSON.stringify({
          version: 2,
    ...
    expect(
      parseChatToVelumPayload(
        JSON.stringify(createChatToVelumPayload("draft", 100)),
  ...
  it("saves and consumes draft handoff once while clearing storage", () => {
    ...
    expect(saveChatToVelumHandoff(storage, "review me", 100)).toBe(true);
    expect(storage.setItem.mock.calls[0][0]).toBe(CHAT_TO_VELUM_HANDOFF_KEY);
    const consumed = consumeChatToVelumHandoff(storage, 200);
    expect(consumed?.draftText).toBe("review me");
    expect(storage.removeItem).toHaveBeenCalledWith(CHAT_TO_VELUM_HANDOFF_KEY);
    expect(data.has(CHAT_TO_VELUM_HANDOFF_KEY)).toBe(false);
  });

  it("keeps Velum-to-Chat redacted handoff unchanged", () => {
