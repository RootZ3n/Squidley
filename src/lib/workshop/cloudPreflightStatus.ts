/**
 * Pure status copy helper for Workshop cloud-required boundary preflight.
 *
 * Returns beginner-friendly messages and a ActivityLog hint based on
...
export interface WorkshopCloudPreflightStatusCopy {
  message: string;
...
export function workshopConsentStatusCopy(
  decision: CloudConsentDecision,
): WorkshopCloudPreflightStatusCopy {
  switch (decision) {
    case "granted":
      return {
        message:
...
const TABULARIUM_HINT = "Recorded in ActivityLog \u2192 Trust chains.";
