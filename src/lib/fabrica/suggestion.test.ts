...
import {
  buildFabricaMessages,
  createFabricaReceiptSummary,
  validateFabricaRequest,
} from "./suggestion";

describe("Fabrica suggestion helpers", () => {
  it("validates a small single-file request", () => {
    const result = validateFabricaRequest({
      fileName: "app.ts",
      language: "typescript",
  ...
  it("requires a requested change and rejects very large files", () => {
    expect(validateFabricaRequest({ originalContent: "", requestedChange: "" })).toMatchObject({ ok: false });
    expect(validateFabricaRequest({ originalContent: "x".repeat(24_001), requestedChange: "fix" })).toMatchObject({ ok: false });
  });

  it("builds strict prompt messages for single-file suggestions", () => {
    const messages = buildFabricaMessages({
      fileName: "index.html",
      language: "html",
  ...
  it("receipt summaries avoid full source and output content", () => {
    const summary = createFabricaReceiptSummary({
      fileName: "secret.ts",
      language: "typescript",
      outputChars: 1200,
    });
