...
export interface FabricaRequest {
  fileName?: string;
  language: string;
...
export type FabricaValidation =
  | { ok: true; value: FabricaRequest }
  | { ok: false; error: string };

export function validateFabricaRequest(input: unknown): FabricaValidation {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    return { ok: false, error: "Fabrica needs a JSON object." };
  }
  ...
  if (originalContent.length > FABRICA_LIMITS.maxOriginalChars) {
    return { ok: false, error: "The pasted file is too long for this beginner Fabrica pass." };
  }
  if (requestedChange.length === 0) {
    return { ok: false, error: "Describe the small single-file change you want Fabrica to suggest." };
  }
...
export function buildFabricaMessages(request: FabricaRequest): ChatMessage[] {
...
export function createFabricaReceiptSummary(args: {
  fileName?: string;
  language: string;
  outputChars?: number;
}): string {
  ...
  return `Fabrica created a local single-file suggestion for ${name} (${args.language}). No files were written and no commands were run.${output}`;
}
