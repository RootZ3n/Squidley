...
import {
  ...
  buildFabricaVelumHandoffPreview,
  buildFabricaVelumHandoffPreparedReceiptInput,
  recordFabricaVelumHandoffPreparedReceipt,
  buildFabricaVelumReviewCompletedPreview,
  buildFabricaVelumReviewCompletedReceiptInput,
  recordFabricaVelumReviewCompletedReceipt,
} from "./velumHandoff";
...
describe("buildFabricaVelumHandoffPreview", () => {
  it("returns nothingSentYet=true and cloudUsed=false", () => {
    const preview = buildFabricaVelumHandoffPreview();
    expect(preview.nothingSentYet).toBe(true);
    expect(preview.cloudUsed).toBe(false);
  });

  it("includes the fabrica:fabrica.multi-file-build capability id", () => {
    const preview = buildFabricaVelumHandoffPreview();
    expect(preview.capabilityId).toBe(FABRICA_MULTI_FILE_BUILD_CAPABILITY_ID);
    expect(preview.capabilityId).toBe("fabrica:fabrica.multi-file-build");
  });
  ...
  it("does not contain raw prompt/code/user/document fields in serialized form", () => {
    const serialized = JSON.stringify(buildFabricaVelumHandoffPreview());
...
describe("buildFabricaVelumHandoffPreparedReceiptInput", () => {
  it("returns a Tabularium-compatible input with correct module and action", () => {
    const input = buildFabricaVelumHandoffPreparedReceiptInput();
...
describe("recordFabricaVelumHandoffPreparedReceipt", () => {
  it("writes only via the supplied storage and returns a receipt", () => {
    ...
    const receipt = recordFabricaVelumHandoffPreparedReceipt(storage);
  ...
  it("does not throw when storage throws (receipt is still built locally)", () => {
    ...
    expect(() =>
      recordFabricaVelumHandoffPreparedReceipt(storage),
    ).not.toThrow();
    // Receipt is still returned (built in memory) even if storage failed
    const result = recordFabricaVelumHandoffPreparedReceipt(storage);
  ...
  it("does not store raw prompt/code/user/document content in persisted data", () => {
    ...
    recordFabricaVelumHandoffPreparedReceipt(storage, { createdAt: 1 });
...
describe("Fabrica Velum handoff preparation — purity", () => {
...
describe("Fabrica Velum handoff preparation — does not unlock gates", () => {
...
describe("buildFabricaVelumReviewCompletedPreview", () => {
  it("returns velumReviewPassed=true", () => {
    const preview = buildFabricaVelumReviewCompletedPreview();
  ...
  it("contains no forbidden keys", () => {
    assertNoForbiddenKeysDeep(buildFabricaVelumReviewCompletedPreview());
  });
});
...
describe("buildFabricaVelumReviewCompletedReceiptInput", () => {
  it("uses the velum-review.completed action", () => {
    const input = buildFabricaVelumReviewCompletedReceiptInput();
  ...
  it("does not contain forbidden keys", () => {
    assertNoForbiddenKeysDeep(buildFabricaVelumReviewCompletedReceiptInput());
  });
...
describe("recordFabricaVelumReviewCompletedReceipt", () => {
  it("writes to storage and returns a receipt with velumReviewPassed=true in metadata", () => {
    const storage = mockStorage();
    const receipt = recordFabricaVelumReviewCompletedReceipt(storage);
    expect(receipt).not.toBeNull();
    expect(receipt!.module).toBe("fabrica");
    expect(receipt!.localOnly).toBe(true);
    expect(receipt!.cloudUsed).toBe(false);
  ...
  it("does not throw when storage throws", () => {
    ...
    expect(() => recordFabricaVelumReviewCompletedReceipt(storage)).not.toThrow();
    const result = recordFabricaVelumReviewCompletedReceipt(storage);
    expect(result).not.toBeNull();
    expect(result!.module).toBe("fabrica");
    expect(result!.localOnly).toBe(true);
    expect(result!.cloudUsed).toBe(false);
  ...
  it("does not store raw prompt/code/user/document content in persisted data", () => {
    ...
    recordFabricaVelumReviewCompletedReceipt(storage, { createdAt: 1 });
    const written = data.get(storage.setItem.mock.calls[0][0])!;
    expect(written).not.toContain('"prompt"');
    expect(written).not.toContain('"rawText"');
    expect(written).not.toContain('"draftText"');
...
describe("Fabrica Velum — prep vs review completion distinction", () => {
...
describe("Fabrica Velum review completion — purity", () => {
  let originalFetch: typeof globalThis.fetch;
  let fetchCalled: boolean;
  ...
  it("no fetch calls when recording review completed receipt", () => {
    const storage = mockStorage();
    recordFabricaVelumReviewCompletedReceipt(storage);
    expect(fetchCalled).toBe(false);
  });
});
