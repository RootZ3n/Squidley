...
import { summarizeLocalCapabilities } from "@/lib/insights/localCapabilitySummary";
...
export default function InsightsPage() {
  const [preferences, setPreferences] = useState<InsightsModelPreferencesDocument>(() =>
    createModelPreferencesDocument(),
  );
  ...
  const selectedWorkshopModel = useMemo(
    () => resolveWorkshopBuildModel({
      preferences,
      models: localModels.models,
      configuredModel: localModels.configuredModel,
  ...
  const ratioDecisions = useMemo(
    ...
    [selectedChatModel, selectedWorkshopModel],
  );
  ...
  function updatePreference(moduleId: InsightsPreferenceModuleId, role: InsightsModelRole, model: string) {
  ...
  function handleResetPreferences() {
    ...
    setPreferences(next);
    logActivityReceipt(window.localStorage, buildInsightsModelPreferencesResetReceipt());
  }
      ...
      <TourHighlight target="intro" active={activeTarget}>
        <header className="border-b border-ink-200 pb-5 dark:border-ink-700">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-ink-600 dark:text-ink-300">
            Insights is Peh&rsquo;s map of how the system thinks. It shows which modules exist,
        ...
        <TourHighlight target="insights-asi" active={activeTarget} className="rounded-xl border border-ink-200 bg-white p-4 shadow-sm dark:border-ink-700 dark:bg-ink-800">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-iris-600 dark:text-iris-300">
                Assessment · Adaptive System Intelligence
              </p>
          ...
          <div className="mt-4 grid gap-2">
            {ratioDecisions.map((decision) => (
              <AssessmentDecisionRow key={`${decision.receiptSummary}-${decision.status}`} decision={decision} />
            ))}
          </div>
        </TourHighlight>
...
function ModuleCard({ module }: { module: InsightsModuleCapability }) {
  return (
...
function AssessmentProfileCard(props: {
  label: string;
  model: string;
...
function AssessmentDecisionRow({ decision }: { decision: AssessmentDecision }) {
  return (
    <div className="rounded-lg border border-ink-100 bg-ink-50/70 px-3 py-2 text-sm dark:border-ink-700/60 dark:bg-ink-900/40">
      ...
      <p className="mt-1 text-xs leading-5 text-ink-500 dark:text-ink-300">
        {summarizeAssessmentDecision(decision)}
      </p>
    </div>
  );
}
