...
import {
  ...
  type NotebookRiskSummary,
  type NotebookStatusFilter,
  type NotebookTagFilter,
...
import {
  ...
  buildNotebookBundleImportedReceipt,
  buildNotebookBundleImportFailedReceipt,
  buildNotebookEntryCreatedReceipt,
  buildNotebookEntryDeletedReceipt,
  buildNotebookEntryEditedReceipt,
  buildNotebookEntryExportedReceipt,
  buildNotebookVelumHandoffCreatedReceipt,
  buildNotebookVelumHandoffReceivedReceipt,
} from "@/lib/notebook/receipts";
import {
  consumeVelumToMoreInputHandoff,
  saveMoreInputToVelumHandoff,
} from "@/lib/velum/handoff";
import { archivumFutureSummarizeDecision, archivumLocalStorageDecision } from "@/lib/assessment";
import {
  recordNotebookLocalStorageCapabilityReceipt,
  recordMoreInputLocalStorageCapabilityReceipt,
} from "@/lib/notebook/capabilityReceipts";

const ENTRY_TYPES: NotebookEntryType[] = ["note", "log", "article", "code", "other"];

export default function NotebookPage() {
  const router = useRouter();
  const [doc, setDoc] = useState<NotebookDocument>(() => createNotebookDocument());
  const [title, setTitle] = useState("");
  const [type, setType] = useState<NotebookEntryType>("note");
  const [tagText, setTagText] = useState("");
  const [text, setText] = useState("");
  const [velumReviewed, setVelumReviewed] = useState(false);
  const [riskSummary, setRiskSummary] = useState<NotebookRiskSummary | undefined>();
  const [notice, setNotice] = useState<string | null>(null);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [hasLoaded, setHasLoaded] = useState(false);
  const [query, setQuery] = useState("");
  const [typeFilter, setTypeFilter] = useState<NotebookTypeFilter>("all");
  const [statusFilter, setStatusFilter] = useState<NotebookStatusFilter>("all");
  const [tagFilter, setTagFilter] = useState<NotebookTagFilter>("all");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editTitle, setEditTitle] = useState("");
  const [editType, setEditType] = useState<NotebookEntryType>("note");
  const [editTagText, setEditTagText] = useState("");
  const [editText, setEditText] = useState("");
  const [editVelumReviewed, setEditVelumReviewed] = useState(false);
  const [editRiskSummary, setEditRiskSummary] = useState<NotebookRiskSummary | undefined>();
  ...
  const availableTags = useMemo(() => getNotebookTags(doc.entries), [doc.entries]);
  ...
  useEffect(() => {
    const loaded = loadNotebook(window.localStorage);
    ...
    if (returned) {
      if (returned.entryId && loaded.entries.some((entry) => entry.id === returned.entryId)) {
        ...
        setEditTagText(formatNotebookTags(original?.tags ?? []));
        ...
        logActivityReceipt(window.localStorage, buildNotebookVelumHandoffReceivedReceipt({
          edit: true,
        ...
        setNotice("Redacted text returned from Velum. Review it, then click Save to Notebook when ready.");
        logActivityReceipt(window.localStorage, buildNotebookVelumHandoffReceivedReceipt());
      }
    }
    setHasLoaded(true);
  ...
  useEffect(() => {
    if (!hasLoaded) return;
    saveNotebook(window.localStorage, doc);
  }, [doc, hasLoaded]);
  ...
  function handleReviewInVelum() {
    ...
    logActivityReceipt(window.localStorage, buildNotebookVelumHandoffCreatedReceipt());
  ...
  function handleSave() {
    ...
    setDoc((prev) => upsertNotebookEntry(prev, entry));
    logActivityReceipt(window.localStorage, buildNotebookEntryCreatedReceipt(entry));
    recordMoreInputLocalStorageCapabilityReceipt(window.localStorage);
    setSelectedId(entry.id);
    setNotice(velumReviewed ? "Saved to Notebook with Velum review." : "Saved without Velum review.");
    clearDraft();
  }
  ...
  function startEdit(entry: NotebookEntry) {
    ...
    setEditTagText(formatNotebookTags(entry.tags));
  ...
  function handleSaveEdit() {
    ...
    const result = updateNotebookEntry(doc, editing.id, {
      title: editTitle,
      type: editType,
      text: editText,
      tags: editTagText,
    ...
    setDoc(result.doc);
    logActivityReceipt(window.localStorage, buildNotebookEntryEditedReceipt({
      entryId: editing.id,
      reviewReset: result.reviewReset,
    }));
    recordNotebookLocalStorageCapabilityReceipt(window.localStorage);
  ...
  function handleDelete(entry: NotebookEntry) {
    const ok = window.confirm("Delete this Notebook entry? This only deletes the entry saved in this browser.");
    if (!ok) return;
    setDoc((prev) => deleteNotebookEntry(prev, entry.id));
    logActivityReceipt(window.localStorage, buildNotebookEntryDeletedReceipt(entry.id));
    if (selectedId === entry.id) setSelectedId(null);
    if (editingId === entry.id) cancelEdit();
  }

  function handleExport(entry: NotebookEntry) {
    logActivityReceipt(window.localStorage, buildNotebookEntryExportedReceipt(entry.id));
    const blob = new Blob([formatNotebookExport(entry)], { type: "text/plain;charset=utf-8" });
  ...
  function handleExportAll() {
    logActivityReceipt(window.localStorage, buildNotebookBundleExportedReceipt(doc.entries.length));
    const blob = new Blob([formatNotebookBundle(doc)], { type: "application/json;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
  ...
  async function handleImportFile(file: File | null) {
    ...
    try {
      ...
      if (!preview) {
        setImportError("That file is not a valid Peh Public Notebook Bundle.");
        logActivityReceipt(window.localStorage, buildNotebookBundleImportFailedReceipt());
        return;
      }
  ...
  function handleConfirmImport() {
    ...
    const result = importNotebookBundle(doc, importPreview.bundle);
    setDoc(result.doc);
    setImportPreview(null);
    setImportError(null);
    setNotice(`Imported ${result.importedCount} entries. Imported entries were not automatically reviewed by Velum.`);
    logActivityReceipt(window.localStorage, buildNotebookBundleImportedReceipt(result.importedCount));
    recordNotebookLocalStorageCapabilityReceipt(window.localStorage);
  }
    ...
    <div className="sq-page mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <header className="border-b border-ink-200 pb-5 dark:border-ink-700">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-ink-600 dark:text-ink-300">
          Notebook is your local knowledge shelf. Save notes, snippets, and
        ...
        <nav className="mt-4 flex flex-wrap gap-2 text-xs">
          ...
          <Link href="/chat" className="rounded-lg border border-ink-200 bg-white px-3 py-1.5 font-medium text-ink-700 shadow-sm hover:bg-ink-50 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100">
            Chat
          </Link>
          ...
          <Link href="/activity-log" className="rounded-lg border border-ink-200 bg-white px-3 py-1.5 font-medium text-ink-700 shadow-sm hover:bg-ink-50 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100">
            ActivityLog
          </Link>
          <Link href="/insights" className="rounded-lg border border-ink-200 bg-white px-3 py-1.5 font-medium text-ink-700 shadow-sm hover:bg-ink-50 dark:border-ink-700 dark:bg-ink-800 dark:text-ink-100">
            Insights
          </Link>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <AssessmentCapabilityNote decision={localStorageAssessment} title="Assessment: local storage and search" compact />
        <AssessmentCapabilityNote decision={futureSummarizeAssessment} title="Assessment: future summarize/retrieval" compact />
      </div>
        ...
        >
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_160px]">
            ...
            <label className="text-sm font-medium text-ink-700 dark:text-ink-100">
              ...
              <select
                value={type}
                onChange={(e) => setType(e.target.value as NotebookEntryType)}
                className="mt-1 w-full rounded-lg border border-ink-200 bg-ink-50/60 px-3 py-2 text-sm text-ink-900 outline-none focus:border-squid-300 focus:ring-2 focus:ring-squid-200 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50"
              >
        ...
        >
          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
              <select
                value={typeFilter}
                onChange={(e) => setTypeFilter(e.target.value as NotebookTypeFilter)}
              ...
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value as NotebookStatusFilter)}
                className="rounded-lg border border-ink-200 bg-ink-50/60 px-3 py-2 text-xs text-ink-900 outline-none focus:border-squid-300 focus:ring-2 focus:ring-squid-200 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50"
              >
              ...
              <select
                value={tagFilter}
                onChange={(e) => setTagFilter(e.target.value as NotebookTagFilter)}
                className="rounded-lg border border-ink-200 bg-ink-50/60 px-3 py-2 text-xs text-ink-900 outline-none focus:border-squid-300 focus:ring-2 focus:ring-squid-200 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50"
              >
        ...
        <section className="mt-4 rounded-xl border border-iris-200 bg-white p-4 shadow-sm dark:border-iris-700/60 dark:bg-ink-800">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_160px]">
            ...
            <label className="text-sm font-medium text-ink-700 dark:text-ink-100">
              ...
              <select
                value={editType}
                onChange={(e) => setEditType(e.target.value as NotebookEntryType)}
                className="mt-1 w-full rounded-lg border border-ink-200 bg-ink-50/60 px-3 py-2 text-sm text-ink-900 outline-none focus:border-squid-300 focus:ring-2 focus:ring-squid-200 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50"
              >
                {ENTRY_TYPES.map((entryType) => (
                  <option key={entryType} value={entryType}>
...
function entrySourceLabel(source: string): string {
  if (source === "oculus-analysis") return "Vision analysis";
  if (source === "fabrica-suggestion") return "Workshop suggestion";
  return "manual paste";
}
