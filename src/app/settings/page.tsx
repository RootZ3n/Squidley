...
import { createNotebookDocument, loadNotebook, saveNotebook, type NotebookDocument } from "@/lib/notebook/storage";
import {
...
const TOUR_LINKS = [
  { label: "Restart Chat tour", href: "/chat?tour=1", module: "Chat" },
  { label: "Restart Velum tour", href: "/velum?tour=1", module: "Velum" },
  { label: "Restart Notebook tour", href: "/notebook?tour=1", module: "Notebook" },
  { label: "Restart ActivityLog tour", href: "/tabularium?tour=1", module: "ActivityLog" },
] as const;
...
export default function SettingsPage() {
  ...
  const [archivumDoc, setNotebookDoc] = useState<NotebookDocument | null>(null);
  const [tabulariumDoc, setActivityLogDoc] = useState<ActivityDocument | null>(null);
  ...
  const archivumSummary = useMemo(() => summarizeNotebookStorage(archivumDoc), [archivumDoc]);
  const tabulariumSummary = useMemo(() => summarizeActivityLogStorage(tabulariumDoc), [tabulariumDoc]);

  function recordSettingsReceipt(args: Parameters<typeof logActivityReceipt>[1]) {
    logActivityReceipt(window.localStorage, args);
    setActivityLogDoc(loadActivity(window.localStorage));
  }
  ...
  function handleClearAllChats() {
    const confirmed = window.confirm("Clear all local Chat chats? This only clears chats saved in this browser.");
    if (!confirmed) return;
    ...
    setNotice("All local Chat chats were cleared in this browser.");
  }
  ...
  function handleClearNotebook() {
    const confirmed = window.confirm("Clear all Notebook entries? This only clears Notebook entries saved in this browser.");
    if (!confirmed) return;
    const fresh = createNotebookDocument();
    saveNotebook(window.localStorage, fresh);
    setNotebookDoc(fresh);
    recordSettingsReceipt(buildSettingsNotebookClearedReceipt());
    setNotice("All Notebook entries were cleared in this browser.");
  }
  ...
  function handleExportReceipts() {
    ...
    downloadText(
      ...
      formatActivityExport(tabulariumDoc),
      "text/plain;charset=utf-8",
    );
  ...
  function handleClearReceipts() {
    const confirmed = window.confirm("Clear ActivityLog receipts? This only clears receipts saved in this browser.");
    if (!confirmed) return;
    const fresh = clearActivityReceipts();
    saveActivity(window.localStorage, fresh);
    setActivityLogDoc(fresh);
    setNotice("ActivityLog receipts were cleared in this browser.");
  }
  ...
  return (
    <div className="sq-page mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-2">
        ...
        <Panel title="Notebook Storage">
          <p className="text-sm text-ink-500 dark:text-ink-300">Notebook entries and tags are saved only in this browser.</p>
          <dl className="mt-4 space-y-2 text-sm">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Link href="/notebook" className="rounded-lg border border-ink-200 bg-white px-3 py-2 text-sm font-medium text-ink-700 hover:bg-ink-50 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-100">
              Open Notebook import/export
            </Link>
            <button type="button" onClick={handleClearNotebook} className="rounded-lg border border-amber-200 bg-white px-3 py-2 text-sm font-medium text-amber-800 hover:bg-amber-50 dark:border-amber-700/60 dark:bg-ink-900 dark:text-amber-100">
              Clear all Notebook entries
            </button>
          </div>
        </Panel>

        <Panel title="ActivityLog Receipts">
        ...
        <Panel title="Privacy & Local-Only">
          ...
          <ul className="space-y-2 text-sm text-ink-500 dark:text-ink-300">
            <li>Chat (Chat) uses your configured local model server.</li>
            <li>Safety Check (Velum) runs pattern-based checks in your browser — no AI model call.</li>
            <li>Notes (Notebook) are stored in this browser.</li>
            <li>Activity Log (ActivityLog) receipts are stored in this browser.</li>
