...
import { findReceiptByQueryId } from "@/lib/activity-log/gatewayReceipts";
import { tabulariumLocalReceiptsDecision } from "@/lib/assessment";
...
export default function ActivityLogPage() {
  ...
  const [modelFilter, setModelFilter] = useState<ActivityModelFilter>("all");
  ...
  const localReceiptsAssessment = useMemo(() => tabulariumLocalReceiptsDecision(), []);

  useEffect(() => {
    const loaded = loadActivity(window.localStorage);
  ...
  function handleExport() {
    if (!doc) return;
    const blob = new Blob([formatActivityExport(doc)], { type: "text/plain;charset=utf-8" });
  ...
  function handleClear() {
    const ok = window.confirm("Clear ActivityLog receipts? This only clears receipts saved in this browser.");
    if (!ok) return;
  ...
  return (
    <div className="sq-page mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <TourHighlight target="intro" active={activeTarget}>
        <header className="border-b border-ink-200 pb-5 dark:border-ink-700">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-ink-600 dark:text-ink-300">
            ActivityLog is Peh&rsquo;s receipt room. It shows what happened,
      ...
      <div className="mt-4">
        <AssessmentCapabilityNote decision={localReceiptsAssessment} title="Assessment: local receipts" compact />
      </div>
        ...
        <div>
          <TourHighlight target="tabularium-filters" active={activeTarget} className="rounded-xl border border-ink-200 bg-white p-4 shadow-sm dark:border-ink-700 dark:bg-ink-800">
            <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_150px_150px_150px]">
              ...
              />
              <select value={moduleFilter} onChange={(e) => setModuleFilter(e.target.value as ActivityModuleFilter)} className="rounded-lg border border-ink-200 bg-ink-50/60 px-3 py-2 text-sm text-ink-900 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50">
                {MODULE_FILTERS.map((item) => <option key={item} value={item}>{item === "all" ? "All modules" : item}</option>)}
              </select>
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as ActivityStatusFilter)} className="rounded-lg border border-ink-200 bg-ink-50/60 px-3 py-2 text-sm text-ink-900 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50">
                {STATUS_FILTERS.map((item) => <option key={item} value={item}>{item === "all" ? "All statuses" : item}</option>)}
              </select>
              <select value={modelFilter} onChange={(e) => setModelFilter(e.target.value as ActivityModelFilter)} className="rounded-lg border border-ink-200 bg-ink-50/60 px-3 py-2 text-sm text-ink-900 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50">
...
function ReceiptDetails({ receipt }: { receipt: ActivityReceipt }) {
  return (
