...
import { velumDeterministicReviewDecision } from "@/lib/assessment";
import {
  buildVelumHandoffFromChatReceipt,
  buildVelumHandoffFromMoreInputReceipt,
  buildVelumHandoffToChatReceipt,
...
export default function VelumPage() {
  ...
  const reviewAssessment = useMemo(() => velumDeterministicReviewDecision(), []);
  ...
  useEffect(() => {
    ...
    logActivityReceipt(window.localStorage, buildVelumHandoffFromMoreInputReceipt());
  }, []);
  ...
  function handleReview() {
    ...
    setReview(next);
    setRedacted("");
    logActivityReceipt(window.localStorage, buildVelumReviewReceipt(next));
  ...
  function handleRedact() {
    ...
    logActivityReceipt(window.localStorage, buildVelumRedactionCreatedReceipt());
  }
  ...
  function handleSendToChat() {
    ...
    logActivityReceipt(window.localStorage, buildVelumHandoffToChatReceipt());
    router.push("/chat?from=velum");
  }
  ...
  function handleSendToMoreInput() {
    ...
    logActivityReceipt(window.localStorage, buildVelumHandoffToMoreInputReceipt());
    router.push("/notebook?from=velum");
  }
    ...
    <div className="sq-page mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <div className="mt-4">
        <AssessmentCapabilityNote
          decision={reviewAssessment}
          title="Assessment: deterministic review"
          compact
        />
      </div>
        ...
        <section className="mt-4 rounded-xl border border-ink-200 bg-white p-4 shadow-sm dark:border-ink-700 dark:bg-ink-800">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            ...
            <InfoBox
              ...
              body="Remove or redact risky parts, keep the text local, or manually copy a cleaned version into Chat without including secrets."
            />
          </div>
        </section>
      </TourHighlight>
      ...
      <TourHighlight target="velum-redaction" active={activeTarget}>
        <section className="mt-4 rounded-xl border border-iris-200 bg-iris-50/50 p-4 shadow-sm dark:border-iris-700/60 dark:bg-iris-900/20">
          ...
          <p className="mt-2 text-xs text-iris-800 dark:text-iris-100">
            Only this redacted preview is handed off. Your original pasted text stays on this page. Chat will still wait for you to click Send.
          </p>
          {handoffError && (
