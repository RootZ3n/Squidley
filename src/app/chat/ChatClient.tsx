...
import {
  consumeVelumHandoff,
  mergeVelumDraft,
  saveChatToVelumHandoff,
} from "@/lib/velum/handoff";
import { consumeVisionToChatHandoff } from "@/lib/vision/handoff";
...
import { chatAdvancedPlanningDecision, chatBasicDecision } from "@/lib/assessment";
import {
  buildChatCompletedReceipt,
  buildChatFailedReceipt,
  buildChatSentReceipt,
  buildChatVisionHandoffReceivedReceipt,
  buildChatVelumHandoffCreatedReceipt,
  buildChatVelumHandoffReceivedReceipt,
} from "@/lib/chat/moduleReceipts";
import { recordChatCapabilityDecisionReceipt } from "@/lib/chat/capabilityReceipts";
...
const EXAMPLE_MESSAGES: readonly UiMessage[] = [
  { id: "ex-1", role: "user", text: "Hi Peh — what is this app for?" },
  {
    id: "ex-2",
    role: "assistant",
    text: 'I\'m Peh. This is Chat." We can chat here, and I can teach you the rest of the app as we go.',
    metrics: {
      source: "local",
      model: "example",
      durationMs: 380,
...
export default function ChatClient() {
  ...
  const basicChatAssessment = useMemo(
    () => chatBasicDecision(selectedModel),
    [selectedModel],
  );
  const advancedPlanningAssessment = useMemo(
    () => chatAdvancedPlanningDecision(selectedModel),
    [selectedModel],
  );
  ...
  useEffect(() => {
    ...
    const oculusHandoff = consumeVisionToChatHandoff(window.sessionStorage);
    if (pendingRef.current) return;
    if (handoff) {
      logActivityReceipt(window.localStorage, buildChatVelumHandoffReceivedReceipt());
      setDraft((prev) => {
    ...
    if (oculusHandoff) {
      logActivityReceipt(window.localStorage, buildChatVisionHandoffReceivedReceipt());
      setDraft((prev) => {
        const existing = prev.trim();
        const imported = oculusHandoff.analysisText.trim();
        setImportedDraftNote(
          existing
            ? "Vision analysis appended below your existing draft. Review it, then click Send when you are ready."
            : "Vision analysis imported. Review it, then click Send when you are ready.",
        );
  ...
  const loadLocalStatus = useCallback(
    async (args: { initial?: boolean } = {}) => {
      ...
      try {
        ...
        const preferenceModel = preferences.modules.chat?.chatModel;
  ...
  function handleModelChoiceChange(model: string) {
    ...
    logActivityReceipt(window.localStorage, buildInsightsModelPreferenceChangedReceipt({
      ...
      summary: "The Chat page saved a browser-local preferred chat model. No cloud provider was enabled.",
    }));
  }
  ...
  const send = useCallback(
    async (
      ...
      logActivityReceipt(window.localStorage, buildChatSentReceipt({
        id: receiptId,
        createdAt: startedAt,
        model: selectedModel,
      }));
      recordChatCapabilityDecisionReceipt(window.localStorage, {
        createdAt: startedAt,
        localModels: models,
        selectedModel,
      });
      ...
      const fail = (message: string, safetyReceiptHref?: string) => {
        ...
        logActivityReceipt(window.localStorage, buildChatFailedReceipt({
        ...
        for await (const event of readStreamEvents(response.body)) {
          if (event.type === "meta") {
            ...
            if (event.promptGateway) {
              ...
              if (gatewayReceiptId) safetyReceiptHref = activityReceiptUrl(gatewayReceiptId);
            }
      ...
      logActivityReceipt(window.localStorage, buildChatCompletedReceipt({
  ...
  function handleClearChat() {
    const confirmed = window.confirm(
      "Clear this Chat chat? This only clears the chat saved in this browser.",
    );
    if (!confirmed) return;
    if (pending || !sessionsDoc) return;
  ...
  function handleReviewInVelum() {
    ...
    const ok = saveChatToVelumHandoff(window.sessionStorage, draft);
    if (!ok) {
  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            ...
            <button
              ...
              className="sq-btn"
              style={{ fontSize: 13 }}
              aria-label="Restart the Chat tour"
            >
              ↻ Restart tour
            </button>
          </div>
      ...
      >
        <AssessmentCapabilityNote decision={basicChatAssessment} title="Assessment: basic chat" compact />
        <AssessmentCapabilityNote decision={advancedPlanningAssessment} title="Assessment: advanced planning" compact />
      </div>
            ...
            {importedDraftNote && (
              ...
              >
                ...
                <p style={{ marginTop: 6, opacity: 0.78 }}>
                  Only Velum&rsquo;s redacted preview was imported. Chat
      ...
      <StatusCallout
        ...
        body={
          typeof health.modelCount === "number"
            ...
            : "Chat will stream replies from your local server."
        }
        hint={
          <>
            {selectionNote ? `${selectionNote} ` : ""}
...
function ExampleHeader() {
  return (
    ...
    >
      ...
      This is what Chat looks like. Say hi to Peh and it gets replaced
...
function ReceiptsPanel({
  ...
  return (
    ...
    >
      ...
      >
        Your Chat chat is saved locally in this browser.
      </p>
