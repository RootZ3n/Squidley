...
import type { AssessmentDecision } from "@/lib/assessment";
import { getAssessmentStatusLabel, getAssessmentUnlockLine } from "@/lib/assessment";

export function AssessmentCapabilityNote({
  decision,
  ...
  const unlockLine = getAssessmentUnlockLine(decision);
    ...
    <section className={`rounded-xl border border-ink-200 bg-white/90 p-3 text-sm shadow-sm dark:border-ink-700 dark:bg-ink-800/90 ${className}`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className={ratioStatusClass(decision.status)}>
          {getAssessmentStatusLabel(decision.status)}
        </span>
      </div>
      ...
      {showInsightsLink && (
        <Link href="/insights" className="mt-2 inline-flex text-xs font-medium text-iris-600 underline decoration-dotted underline-offset-4 dark:text-iris-300">
          See Assessment in Insights
        </Link>
      )}
    </section>
  );
...
function ratioStatusClass(status: AssessmentDecision["status"]): string {
